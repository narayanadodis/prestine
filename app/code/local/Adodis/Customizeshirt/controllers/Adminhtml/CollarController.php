...
class Adodis_Customizeshirt_Adminhtml_CollarController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('customizeshirt/collar')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('collar_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('customizeshirt/adminhtml_collar_edit'))
				->_addLeft($this->getLayout()->createBlock('customizeshirt/adminhtml_collar_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('customizeshirt/collar');		
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				...
				$data['collar_image'] = $result['url'];

				$model->setCollarImage($data['collar_image']);
				//$model->save();
    ...
    public function massDeleteAction() {
        $customizeshirtIds = $this->getRequest()->getParam('collar');
        if(!is_array($customizeshirtIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getModel('customizeshirt/collar')->load($customizeshirtId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($customizeshirtIds)) {
            ...
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getSingleton('customizeshirt/collar')
    ...
    {
        $fileName   = 'collar.csv';
        $content    = $this->getLayout()->createBlock('customizeshirt/adminhtml_collar_grid')
            ->getCsv();
	...
	protected function uploadFile( $file_name ){
		...
		try {
			$result = array();
			$dynamicScmsURL = 'customizeshirt/collar';
			$baseScmsMediaURL = Mage::getBaseUrl('media') . DS . 'customizeshirt/collar';
			$baseScmsMediaPath = Mage::getBaseDir('media') . DS .  'customizeshirt/collar';
