...
class Adodis_Customizeshirt_Adminhtml_MonolabelController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('customizeshirt/monolabel')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('monolabel_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('customizeshirt/adminhtml_monolabel_edit'))
				->_addLeft($this->getLayout()->createBlock('customizeshirt/adminhtml_monolabel_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('customizeshirt/monolabel');		
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				...
				$data['monolabel_image'] = $result['url'];

				$model->setMonolabelImage($data['monolabel_image']);
				//$model->save();
    ...
    public function massDeleteAction() {
        $customizeshirtIds = $this->getRequest()->getParam('monolabel');
        if(!is_array($customizeshirtIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getModel('customizeshirt/monolabel')->load($customizeshirtId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($customizeshirtIds)) {
            ...
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getSingleton('customizeshirt/monolabel')
    ...
    {
        $fileName   = 'monolabel.csv';
        $content    = $this->getLayout()->createBlock('customizeshirt/adminhtml_monolabel_grid')
            ->getCsv();
	...
	protected function uploadFile( $file_name ){
		...
		try {
			$result = array();
			$dynamicScmsURL = 'customizeshirt/monolabel';
			$baseScmsMediaURL = Mage::getBaseUrl('media') . DS . 'customizeshirt/monolabel';
			$baseScmsMediaPath = Mage::getBaseDir('media') . DS .  'customizeshirt/monolabel';
