...
class Adodis_Customizeshirt_Adminhtml_EmbroideryController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('customizeshirt/embroidery')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('embroidery_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('customizeshirt/adminhtml_embroidery_edit'))
				->_addLeft($this->getLayout()->createBlock('customizeshirt/adminhtml_embroidery_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('customizeshirt/embroidery');		
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				...
				$data['embroidery_image'] = $result['url'];

				$model->setEmbroideryImage($data['embroidery_image']);
				//$model->save();
    ...
    public function massDeleteAction() {
        $customizeshirtIds = $this->getRequest()->getParam('embroidery');
        if(!is_array($customizeshirtIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getModel('customizeshirt/embroidery')->load($customizeshirtId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($customizeshirtIds)) {
            ...
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getSingleton('customizeshirt/embroidery')
    ...
    {
        $fileName   = 'embroidery.csv';
        $content    = $this->getLayout()->createBlock('customizeshirt/adminhtml_embroidery_grid')
            ->getCsv();
	...
	protected function uploadFile( $file_name ){
		...
		try {
			$result = array();
			$dynamicScmsURL = 'customizeshirt/embroidery';
			$baseScmsMediaURL = Mage::getBaseUrl('media') . DS . 'customizeshirt/embroidery';
