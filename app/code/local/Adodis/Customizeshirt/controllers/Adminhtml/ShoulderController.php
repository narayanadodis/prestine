...
class Adodis_Customizeshirt_Adminhtml_ShoulderController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('customizeshirt/shoulder')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('shoulder_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('customizeshirt/adminhtml_shoulder_edit'))
				->_addLeft($this->getLayout()->createBlock('customizeshirt/adminhtml_shoulder_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('customizeshirt/shoulder');		
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				...
				$data['shoulder_image'] = $result['url'];

				$model->setShoulderImage($data['shoulder_image']);
				//$model->save();
    ...
    public function massDeleteAction() {
        $customizeshirtIds = $this->getRequest()->getParam('shoulder');
        if(!is_array($customizeshirtIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getModel('customizeshirt/shoulder')->load($customizeshirtId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($customizeshirtIds)) {
            ...
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getSingleton('customizeshirt/shoulder')
    ...
    {
        $fileName   = 'shoulder.csv';
        $content    = $this->getLayout()->createBlock('customizeshirt/adminhtml_shoulder_grid')
            ->getCsv();
	...
	protected function uploadFile( $file_name ){
		...
		try {
			$result = array();
			$dynamicScmsURL = 'customizeshirt/shoulder';
			$baseScmsMediaURL = Mage::getBaseUrl('media') . DS . 'customizeshirt/shoulder';
			$baseScmsMediaPath = Mage::getBaseDir('media') . DS .  'customizeshirt/shoulder';
