...
class Adodis_Customizeshirt_Adminhtml_PocketController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('customizeshirt/pocket')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('pocket_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('customizeshirt/adminhtml_pocket_edit'))
				->_addLeft($this->getLayout()->createBlock('customizeshirt/adminhtml_pocket_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('customizeshirt/pocket');		
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				...
				$data['pocket_image'] = $result['url'];

				$model->setPocketImage($data['pocket_image']);
				//$model->save();
    ...
    public function massDeleteAction() {
        $customizeshirtIds = $this->getRequest()->getParam('pocket');
        if(!is_array($customizeshirtIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getModel('customizeshirt/pocket')->load($customizeshirtId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($customizeshirtIds)) {
            ...
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getSingleton('customizeshirt/pocket')
    ...
    {
        $fileName   = 'pocket.csv';
        $content    = $this->getLayout()->createBlock('customizeshirt/adminhtml_pocket_grid')
            ->getCsv();
	...
	protected function uploadFile( $file_name ){
		...
		try {
			$result = array();
			$dynamicScmsURL = 'customizeshirt/pocket';
			$baseScmsMediaURL = Mage::getBaseUrl('media') . DS . 'customizeshirt/pocket';
			$baseScmsMediaPath = Mage::getBaseDir('media') . DS .  'customizeshirt/pocket';
