...
class Adodis_Customizeshirt_Adminhtml_SleeveController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() 
	{
		...
		$model  = Mage::getModel('customizeshirt/sleeve')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('sleeve_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('customizeshirt/adminhtml_sleeve_edit'))
				->_addLeft($this->getLayout()->createBlock('customizeshirt/adminhtml_sleeve_edit_tabs'));
	...
	public function saveAction() 
	{
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('customizeshirt/sleeve');		
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				...
				$data['sleeve_image'] = $result['url'];

				$model->setSleeveImage($data['sleeve_image']);
				//$model->save();
	...
	{
        $customizeshirtIds = $this->getRequest()->getParam('sleeve');
        if(!is_array($customizeshirtIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getModel('customizeshirt/sleeve')->load($customizeshirtId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($customizeshirtIds)) {
            ...
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getSingleton('customizeshirt/sleeve')
    ...
    {
        $fileName   = 'sleeve.xml';
        $content    = $this->getLayout()->createBlock('customizeshirt/adminhtml_sleeve_grid')
            ->getXml();
	...
	protected function uploadFile( $file_name )
	{
		...
		try {
			$result = array();
			$dynamicScmsURL = 'customizeshirt/sleeve';
			$baseScmsMediaURL = Mage::getBaseUrl('media') . DS . 'customizeshirt/sleeve';
			$baseScmsMediaPath = Mage::getBaseDir('media') . DS .  'customizeshirt/sleeve';
