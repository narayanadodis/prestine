...
class Adodis_Customizeshirt_Adminhtml_ButtonController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('customizeshirt/button')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('customizeshirt/adminhtml_button_edit'))
				->_addLeft($this->getLayout()->createBlock('customizeshirt/adminhtml_button_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('customizeshirt/button');		
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				...
				$data['button_image'] = $result['url'];

				$model->setButtonImage($data['button_image']);
				//$model->save();
    ...
    public function massDeleteAction() {
        $customizeshirtIds = $this->getRequest()->getParam('button');
        if(!is_array($customizeshirtIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getModel('customizeshirt/button')->load($customizeshirtId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($customizeshirtIds)) {
            ...
            try {
                foreach ($customizeshirtIds as $customizeshirtId) {
                    $customizeshirt = Mage::getSingleton('customizeshirt/button')
    ...
    {
        $fileName   = 'button.csv';
        $content    = $this->getLayout()->createBlock('customizeshirt/adminhtml_button_grid')
            ->getCsv();
	...
	protected function uploadFile( $file_name ){
		...
		try {
			$result = array();
			$dynamicScmsURL = 'customizeshirt/button';
			$baseScmsMediaURL = Mage::getBaseUrl('media') . DS . 'customizeshirt/button';
			$baseScmsMediaPath = Mage::getBaseDir('media') . DS .  'customizeshirt/button';
