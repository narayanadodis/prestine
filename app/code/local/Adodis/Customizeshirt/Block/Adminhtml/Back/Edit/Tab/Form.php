...
class Adodis_Customizeshirt_Block_Adminhtml_Back_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
  protected function _prepareForm()
  {
	$model = Mage::registry('back_data');
      $form = new Varien_Data_Form();
      $this->setForm($form);
      $fieldset = $form->addFieldset('customizeshirt_form', array('legend'=>Mage::helper('customizeshirt')->__('Item information')));
     
      $fieldset->addField('back_title', 'text', array(
          'label'     => Mage::helper('customizeshirt')->__('Back Title'),
          'class'     => 'required-entry',
          'required'  => true,
          'name'      => 'back_title',
      ));

      $object = Mage::getModel('customizeshirt/back')->load( $this->getRequest()->getParam('id') );
		$note = false;
		$img = '';
		...
		if( $object["back_image"] ){
			...
			$config = Mage::getModel('customizeshirt/back');
			$image = Mage::getModel('customizeshirt/back');
			$image->setConfig($config);

			$imageFile =  Mage::getBaseUrl('media').$object["back_image"];
			$strimg = $object["back_image"];
	  ...
	  $fieldset->addField('back_image', 'file', array(
			'label'        => Mage::helper('customizeshirt')->__('Back Image'),
			'note'      => $note,
			'name'        => 'filename',
	  ...
	  $fieldset->addField('switchno', 'text', array(
          'label'     => Mage::helper('customizeshirt')->__('Back Switch_no'),
          'class'     => 'required-entry',
          'required'  => true,
          'name'      => 'switchno',
      ));
